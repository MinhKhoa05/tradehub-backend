...
### [2026-05-08]
**Topic: Resolving Flaky Integration Tests (Parallelism & Hardcoded Context)**
- **Type**: Bug Fix / Testing Best Practice
- **Tags**: [testing] [xunit] [parallelism] [flaky-tests]
- **Scope**: Integration Tests (CustomWebApplicationFactory, OrderApiTests, UserApiTests)
- **Impact**: Khắc phục dứt điểm tình trạng 404 (NotFound) ngẫu nhiên khi chạy tests qua Visual Studio Test Explorer, đảm bảo bộ test ổn định 100%.

**Bài học rút ra**:
- **Test Runner Parallelism vs SQLite In-Memory**: Mặc dù đã dùng `[Collection("IntegrationTests")]` để xUnit chạy tuần tự, một số Test Runner (như Visual Studio) có thể ghi đè behavior này và chạy các file test song song. Kết quả là `ResetDatabaseAsync` (xóa trắng bảng) ở Test B vô tình thực thi ngay giữa lúc Test A đang chạy, dẫn đến việc Dữ liệu vừa Seed bị bốc hơi -> Lỗi `404 NotFound`.
- **Triệt để tắt Parallelism**: Để đảm bảo DB in-memory không bị overlap, bắt buộc phải cấm song song ở cấp độ Assembly bằng cách khai báo: `[assembly: CollectionBehavior(DisableTestParallelization = true)]` tại file cấu hình Collection.
- **Tránh "ID Cứng" (Hardcoded Context ID) trong API Tests**: Trong TestAuthHandler, nếu có cấu hình Mock User bằng Header (ví dụ fallback `UserId = 1`), các request `_client.PostAsJsonAsync` sẽ mặc định dùng `UserId = 1`. Tuy nhiên, sau mỗi lần Reset DB, ID được sinh ra từ `SeedUserAsync` có thể là 2, 3... Khi đó API sẽ tìm User 1 không thấy -> `404 NotFound`.
- **Explicit Test Headers**: Thay vì dùng `PostAsJsonAsync` thiếu Header, hãy khởi tạo request rõ ràng bằng `HttpRequestMessage` và truyền chính xác `X-Test-UserId` vừa được trả về từ hàm Seed. 
